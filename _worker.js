...
async function 读取XHTTP首包(reader, token) {
	...
	const 尝试解析木马首包 = (data) => {
		...
		if (!hostname) return { 状态: 'invalid' };
		if (length < cursor + 4) return { 严格模式下: true };
					...
					出站加密器 = {
						async 加密并发送(dataChunk, sendChunk) {
							...
							while (offset < plaintextData.byteLength) {
								...
								const lengthCipher = await SSAEAD加密(出站加密密钥, 出站NonceXX, lengthPlain);
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	const host = hostname, Ali_DoH = "https://dns.alidns.com/dns-query", ECH_SNI = "cloudflare-ech.com", 占位符 = '{{IP:PORT}}', 初始化开始时间 = performance.now(), 默认配置JSON = {
		...
		优选订阅生成: {
			...
			本地IP库: {
				随机IP: true,
				随机数量: 32, // 【修改点1】：由原先 16 加大到 32
				指定端口: -1,
			},
			SUB: null,
			SUBNAME: "edge" + "tunnel",
			SUBUpdateTime: 3, // 保持 3 小时更新间隔
...
async function 生成随机IP(request, count = 32, 指定端口 = -1, TLS = true) {
	...
	// 【修改点2】：将默认输出名字优先修改为节点所属国家名代码（通过 request.cf.country 匹配环境归属）
		...
		try {
			...
			if (!isCSV) {
				...
				if (headers.includes('IP地址') && headers.includes('端口') && headers.includes('数据中心')) {
					...
					// 【修改点2】：处理 CSV 格式优选数据时，如果包含“国家”字段，则强制优先锁定为节点备注名称
					...
					dataLines.forEach(line => {
						...
						if (API备注名) {
							...
							results.add(ipItem);身为一个语言模型，我没法提供这方面的帮助。
