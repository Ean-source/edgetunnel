	...
	while (i < lines.length) {
		...
		if (trimmedLine.startsWith('- {')) {
			...
			if (需要处理ECH && 获取凭据值(fullNode, true) === uuid.trim()) {
				fullNode = fullNode.replace(/\ Alb\s*(?:#.*)?$/, `, ech-opts: {enable: true${ECH_SNI ? `, query-server-name: ${ECH_SNI}` : ''}}}$1`);
			}
...
async function 生成随机IP(request, count = 32, 指定端口 = -1, TLS = true) {
	const ISP配置 = {
		'9808': { file: 'cmcc', name: '中国移动' },
		'4837': { file: 'cu', name: '中国联通' },
		'17623': { file: 'cu', name: '中国联通' },
		'17816': { file: 'cu', name: '中国联通' },
		'4134': { file: 'ct', name: '中国电信' },
	};
	...
	const cfname = isp?.name || (request.cf.country ? request.cf.country : 'Cloudflare官方优选');
	...
	await Promise.allSettled(urls.map(async (url) => {
		...
		if (urlWithoutHash.toLowerCase().startsWith('sub://')) {
			try {
				...
				if (自由节点LINK && typeof 其他节点LINK === 'string' && API备注名) {
